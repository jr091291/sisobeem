...
public class BuildAgents extends BuildAgentsAbstract {
	...
	public ArrayList<IComponentIdentifier> createTranseuntes(PersonsConfig personConfig) {
		...
		if (personConfig.getTranseuntes() > 0) {
			...
			int[] edades = Random.getEdadRandom(personConfig.getTranseuntes(), porcentajesEdades[0],
					porcentajesEdades[1] - porcentajesEdades[0], 100 - porcentajesEdades[1]);
			...
			double[] conocimiento = Random.getConocimientoRandom(personConfig.getTranseuntes(),
					porcentajesConocimiento[0], porcentajesConocimiento[1] - porcentajesConocimiento[0],
			...
			return (ArrayList<IComponentIdentifier>) super.getPlataforma()
					.BuildComponents("sisobeem.agent.person/CivilAgentBDI.class", infos).clone();
		}
	...
	public ArrayList<IComponentIdentifier> createTranseuntes(PersonsConfig personConfig, int cantidad) {
		...
		if (cantidad > 0) {
			...
			double[] conocimiento = Random.getConocimientoRandom(cantidad, porcentajesConocimiento[0],
					porcentajesConocimiento[1] - porcentajesConocimiento[0], 100 - porcentajesConocimiento[1]);
	...
	public ArrayList<IComponentIdentifier> createEdifices(EdificesConfig[] edificesConfigs) {
		...
		if (edificesConfigs.length > 0) {
			...
			for (EdificesConfig edificesConfig : edificesConfigs) {
				...
				Map<String, Object> arguments = new HashMap<String, Object>(); // Mapa
																				// de
																				// Argumentos
				...
				ArrayList<IComponentIdentifier> personas = createTranseuntes(super.getConfig().getPersonsConfig(),
						edificesConfig.getPersonas());
	 ...
	 * 
	 * @param personas
	 *            Las personas que se seran asignadas aleatoriamente en los
	 *            pisos creados
	 * @param numPisos
	 *            Numero de pisos de la edificacion
	 * @return
	 */
	@SuppressWarnings("unchecked")
		...
		if (numPisos > 0) {
			...
			for (int i = 0; i < numPisos; i++) {
				...
				Map<String, Object> arguments = new HashMap<String, Object>(); // Mapa
																				// de
																				// Argumentos
				ArrayList<IComponentIdentifier> personPlants = new ArrayList<IComponentIdentifier>(); // Personas
																										// para
																										// el
																										// piso
	...
	public ArrayList<IComponentIdentifier> createAtencionEmergency(EmergencyConfig emergencyConfig) {
		...
		return  (ArrayList<IComponentIdentifier>) super.getPlataforma()
				.BuildComponents("sisobeem.agent.emergency/CoordinadorEmergenciaAgentBDI.class", infos).clone();
	}
	...
	private ArrayList<IComponentIdentifier> createBusqueda(int cantidad) {
		...
		if (cantidad > 0) {
			...
			return (ArrayList<IComponentIdentifier>) super.getPlataforma()
					.BuildComponents("sisobeem.agent.emergency/BusquedaYRescateAgentBDI.class", infos).clone();
		}
		return new ArrayList<IComponentIdentifier>();
	...
	private ArrayList<IComponentIdentifier> CreateSalud(int cantidad) {
		...
		if (cantidad > 0) {
			...
			return (ArrayList<IComponentIdentifier>) super.getPlataforma()
					.BuildComponents("sisobeem.agent.emergency/SaludAgentBDI.class", infos).clone();
		}
		return new ArrayList<IComponentIdentifier>();
	...
	private ArrayList<IComponentIdentifier> createSeguridad(int cantidad) {
		...
		if (cantidad > 0) {
			...
			return (ArrayList<IComponentIdentifier>) super.getPlataforma()
					.BuildComponents("sisobeem.agent.emergency/SeguridadAgentBDI.class", infos).clone();
		}
		return new ArrayList<IComponentIdentifier>();
