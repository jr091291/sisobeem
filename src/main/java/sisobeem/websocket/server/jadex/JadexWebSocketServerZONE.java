...
import static sisobeem.websocket.Session.DeviceSessionHandlerZONE.getDeviceSessionHandler;
...
import sisobeem.websocket.Session.DeviceSessionHandler1;
import sisobeem.websocket.Session.DeviceSessionHandlerZONE;

@ApplicationScoped
@ServerEndpoint(value="/simulacion/jadexZONE")
public class JadexWebSocketServerZONE{
       
	 
	 private DeviceSessionHandlerZONE sessionHandler = getDeviceSessionHandler();
	 ...
	 public void close(Session session, CloseReason reason) {
		 Log.getLog().setInfo("Se ha cerrado una conexion con el socket: " + this.getClass().getName() + " " + reason.getReasonPhrase());
	 }
